...
#origem para treinamento do modelo com 6 milhões de registros
INPUT_CSV = Path("result_db/origem_600_features.csv")
OUTPUT_DIR = Path("saida_features_aa_dipep_v3_seed_6milhoes")

#origem para treinamento do modelo com 500 mil linhas de amostra
#INPUT_CSV = Path("result_db/resultado_debug_500k.csv")
#OUTPUT_DIR = Path("saida_features_aa_dipep_v3_seed")
...
WINDOW_SIZE = 5
DIPEPTIDE_DIM = 400
FULL_SEED_SPACE = 160000  # 400 x 400
...
TOTAL_DIM = AA_DIM + DIPEPTIDE_DIM + EXTRA_DIM + SEED_STATS_DIM
...
X_DAT = OUTPUT_DIR / "X_features_v3_seed_float32.dat"
...
def build_feature_names() -> list[str]:
    ...
    return aa_cols + dipep_cols + extra_cols + SEED_FEATURE_NAMES
...
def sequence_to_features(seq: str) -> np.ndarray:
    ...
    feat[AA_DIM + DIPEPTIDE_DIM + EXTRA_DIM:] = seed_features
